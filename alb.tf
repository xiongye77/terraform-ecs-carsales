...
resource "aws_lb_listener_rule" "dfsc_test2_https" {
  listener_arn = aws_lb_listener.dfsc_https.arn
  priority     = 200
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.dfsc-back-end-tg-2.arn
  }
    condition {
    path_pattern {
      values = ["/test2/"]
...
resource "aws_lb_target_group" "dfsc-back-end-tg-2" {
  name                 = "demo-http-blue"
  port                 = "3000"
  protocol             = "HTTP"
  target_type          = "ip"
